...
//HPConsumeSkill
//3、行动后角色着色
//Drill_CoreOfFilter、Drill_EventFilter
//=============================================================================
...
//版本更新时间：2018/07/26
//verision update time：2018/07/26
//更新版本号：0.1.3a.7.26.19.19.39
//UPDATE CODE：0.1.3a.7.26.19.19.39
//Released under the MIT license.
// http://opensource.org/licenses/mit-license.php
...
//=============================================================================
//update:back attatck and counter attack updated by @Tomoaki
//       fixed some bugs coursed by clashing by Tomoaki's update and my update
//更新：原作者@Tomoaki对脚本进行了更新，增加了背刺和反击等
//      解决了作者更新的部分与本人修改部分直接冲突产生的bug
//=============================================================================
...
//=============================================================================
//更新：
//		1.TODO:新增支援<support:xx>和掩护<cover:xx>
//             一回合一次，xx是触发概率
//=============================================================================
 ...
 * 
 *   <srpgEscape>
 *   このタグのついたイベントをSRPG逃走イベントとして利用します。
 *
 *   <srpgActor:0>
 ...
 *   自動的に付加します。
 *
 *   设置单位的时候，第二页打开独立开关A，然后选择无图标+在人物之下
 * 
...
(function() {
  ...
  var escapeCommand = parameters['escapeCommand'];
  ...
  Game_Action.prototype.srpgApply = function(target) {
    ...
    if (result.isHit()) {
	  ...
	  if (this.item().meta.back) { //击退和抗性,back/dot_back
        var targetEvent = target.srpgEvent();
        var subjectEvent = this.subject().srpgEvent();
        var lastDirection = subjectEvent.direction();
		var srpgActorId = targetEvent.event().meta.srpgActor;
        var back_x = targetEvent.x;
        var back_y = targetEvent.y;
        var i, equip;
		value = this.item().meta.back;

		if (srpgActorId) {
		  var actor = $gameParty.allMembers()[+srpgActorId];
		  if (actor) {
			for (i = 0; i < actor._equips.length; i++) {
				if (actor._equips[i]._dataClass === "weapon") {
					equip = $dataWeapons[actor._equips[i]._itemId];
				} else if (actor._equips[i]._dataClass === "armor") {
					equip = $dataArmors[actor._equips[i]._itemId];
				}
				if (equip && equip.meta.dot_back)
					value = 0;
			}
		  } else {
				value = 0;
		  ...
		  var srpgEnemyId = targetEvent.event().meta.srpgEnemy;
		  if (srpgEnemyId != null) {
			if (+srpgEnemyId > 0) {
			  equip = $dataEnemies[+srpgEnemyId]
			  if (equip && equip.meta.dot_back)
				value = 0;
			}
		  }
        ...
        subjectEvent.turnTowardCharacter(targetEvent);//2 (down), 4 (left), 6 (right), or 8 (up).
		...
		for (; value > 0; value--) {
			targetEvent.moveTowardCharacter({x:back_x, y:back_y});
		}
	  }
  ...
  Game_Action.prototype.srpgMakeDamageValue = function(target, evaluate, critical) {
	...
	if (this.item().meta.penetration) { //贯通,penetration
		value = baseValue;
	}
...
/*	this.events().filter(function(event) {
    ...
    this.setupSrpgEscapeEvent();          // SRPG逃走イベントのセットアップ
  ...
  // 新增逃跑点
  Game_Map.prototype.setupSrpgEscapeEvent = function() {
    var events = this.events();
    for (var i = 0; i < events.length; i++) {
      var event = events[i];
      if (event.event().meta.srpgEscape) {
        this._srpgEscapeEventId = event.eventId();
        event.refresh();
        break;
      }
    }
  ...
  // SRPG逃走イベントを返す
  Game_Map.prototype.srpgEscapeEvent = function() {
    return this.event(this._srpgEscapeEventId);
  };
  // SRPG逃走イベントを起動する
  Game_Map.prototype.startSrpgEscapeEvent = function() {
    this.srpgEscapeEvent().start();
  };
  // SRPG逃走判断
  Game_Map.prototype.SrpgEscapeCheck = function(event) {
    if (this.srpgEscapeEvent().x === event.x && this.srpgEscapeEvent().y === event.y)
		return this.srpgEscapeEvent().x === event.x && this.srpgEscapeEvent().y === event.y;
  };
  ...
  // 範囲内のSRPGユニットを返す，新增soulsaga的敌友伤害区分
  // 新增无差别伤害
  Game_Map.prototype.srpgUnitsArea = function(area, alive, act, skill) {
    area = area || this._srpgArea;
    var result = [];
    var events = [];
    //var noatt = 0;
	events = this.srpgUnits(alive);
	if(skill && skill.meta.no_diff) {
      for (var i = 0; i < events.length; i++) {
		var event = events[i];
      ...
      for (var i = 0; i < events.length; i++) {
		var event = events[i];
		var key = '' + event.x + ',' + event.y;
		//if (skill != 0 && area[key] && act.isFriend(event) && skill.damage.type != 3) noatt += 1;
		if (skill != 0 && area[key] && act.isFriend(event) && skill.damage.type === 3) result.push(event);
		if (skill != 0 && area[key] && !act.isFriend(event) && skill.damage.type != 3) result.push(event);
		if (skill === 0 && area[key]) result.push(event);
      }
	}
    return result;
  };
  ...
  Game_Map.prototype.checkMovableArea = function(unit) {
    var mov = unit.mov();
	var srpgActorId = unit.event().meta.srpgActor;
  ...
  // 反撃表示する
  Game_Map.prototype.showCounterArea = function(unit) {
    this._srpgArea = this.createArea(unit.x, unit.y, ['rect', 0]);
    this.setColorArea(colorAreaRange);
  };
  
  // 射程範囲を表示する
  Game_Map.prototype.showRangeArea = function(event_act, skill) {
    this._srpgArea = this.createRangeArea2(event_act, skill);
    this.setColorArea(colorAreaRange);
  };

  // 射程範囲を作成する
  Game_Map.prototype.createRangeArea2 = function(event_act, skill) {
	var area = this.createRangeArea(event_act.x, event_act.y, skill);
	if(skill.meta.no_self) {
      this.deleteArea(area, this.isAreaValidRect, event_act._x, event_act._y, 0, false);
	}
    return area;
  };
  ...
  // 事件待机黑白滤镜
  Game_Event.prototype.SrpgEventFilter = function(flag) {
	// 确认是否有加入行走图渲染
	if (!Imported.Drill_EventFilter) return;
	if (!flag) {
  ...
  Game_Event.prototype.checkSearch = function() {
    ...
    var events = $gameMap.srpgUnitsArea(area, true, this, 0);
  ...
  Game_Event.prototype.showSrpgActionAnimation = function() {
    ...
    var targets = $gameMap.srpgUnitsArea(null, true, this, subject.currentAction().item());
  ...
  Game_Event.prototype.executeSrpgAction = function() {
    $gameTemp.setSrpgNextUnitId(-this.eventId());
      ...
      this.srpgCheckDead();
    var action = subject.currentAction();
    var checkFlag = action.isSkill() && action.item().id === checkSkillId;// && this._actor.canUse(action.item());
    var targets = $gameMap.srpgUnitsArea(null, true, this, action.item());
    for (var i = 0; i < targets.length; i++) {
  ...
  Game_Event.prototype.srpgCheckCounter = function(action, attacker) {
    ...
    if (Math.random() < action.itemCnt(battler) && !(action.item().meta.penetration)) {
  ...
  Game_Event.prototype.srpgCheckReflection = function(action) {
    var battler = this.srpgBattler();
    if (Math.random() < action.itemMrf(battler) && !(action.item().meta.penetration)) {
  ...
  Window_SrpgCommand.prototype.makeCommandList = function() {
    ...
    if (this._srpgUnit.isSrpgInputtable()) {
	  ...
	  var srpgActorId = this._srpgUnit.event().meta.srpgActor;
	  var actor = $gameParty.allMembers()[+srpgActorId];

	  if(actor._states.length > 0) { //新增dot_act状态信息
		  if($dataStates[actor._states].meta.dot_act)
			  canAct = false;
	  }

	  if (this._srpgUnit.mov() === 0) {
		  this.addCommand(moveCommand, 'move', false);
	  } else {
		  if (canMove || (this._srpgUnit.isLastPositionValid() === false && canAct === true)) { // 修复0移动力异常报错
			if (this._srpgUnit.isLastPositionValid() === false && canAct === true) { // 0移动力不可移动
			...
			if (escapeCommand && $gameMap.SrpgEscapeCheck(this._srpgUnit)) this.addCommand(escapeCommand, 'escape', true);
		  }
	  }
      this.addCommand(actionCommand, 'action', canAct);
      if (itemCommand) this.addCommand(itemCommand, 'item', canAct);
      if (equipCommand) this.addCommand(equipCommand, 'equip', canAct && canMove);
	}
  ...
  Window_SrpgStatus.prototype.makeDamageText = function(targetBattler, action) {
	...
	if (action.item().meta.penetration || action.item().meta.no_diff) { //贯通,penetration
		cnt = 0;
		mrf = 0;
	}
  ...
  Window_SrpgSkillList.prototype.isEnabled = function(item) {
	var no_move = 0;
	if (item.meta.no_move)
		no_move = $gameMap.event(this._actor._srpgEventId)._moved;
    return this._actor && this._actor.canUse(item) && !no_move;
  };
...
//
  ...
  Scene_Map.prototype.createSrpgCommandWindow = function() {
    ...
    this._srpgCommandWindow.setHandler('escape', this.srpgCommandEscape.bind(this));
  ...
  Scene_Map.prototype.updateSrpgCounterUnit = function() {
    if (this._counterUnit) {
      ...
      if (eventId > 0 && eventId != $gameTemp.srpgCommandEvent().eventId()) {
        this._counterUnit = $gameMap.event(eventId);
		$gameMap.showCounterArea(this._counterUnit);
  ...
  // SRPGコマンド【逃走】
  Scene_Map.prototype.srpgCommandEscape = function() {
    this._srpgStatusWindow.refresh();
    this.cancelSrpgCommand();
	$gameMap.startSrpgEscapeEvent();
  };
  ...
  Scene_Map.prototype.srpgCommandActionNo = function() {
	...
	$gameMap.initColorArea();
    $gameMap.showRangeArea(event, skill);
  ...
  Scene_Map.prototype.okSrpgSkill = function() {
    ...
    this.setAreaHandler(this.okAreaAction, this.cancelAreaAction);
    $gameMap.showRangeArea(event, this._srpgSkillWindow.item());
  };
  ...
  Scene_Map.prototype.okAreaAction = function(x, y) {
    ...
    var events = $gameMap.srpgUnits(true);
    var skill = this._srpgSkillWindow.item();
	var area = $gameMap.createEffectArea(x, y, this._srpgSkillWindow.item());
    var noatt = 0;
	var result = [];

	
	area = area || $gameMap._srpgArea;
	if(skill && skill.meta.no_diff) {
      for (var i = 0; i < events.length; i++) {
		var events_sets = events[i];
		var key = '' + events_sets.x + ',' + events_sets.y;
		if (area[key]) result.push(events_sets);
      }
	} else {
		for (var i = 0; i < events.length; i++) {
		var events_sets = events[i];
		var key = '' + events_sets.x + ',' + events_sets.y;
		//if (skill != 0 && area[key] && event.isFriend(events_sets) && skill.damage.type != 3) noatt += 1;
		if (skill != 0 && area[key] && event.isFriend(events_sets) && skill.damage.type === 3) result.push(events_sets);
		if (skill != 0 && area[key] && !event.isFriend(events_sets) && skill.damage.type != 3) result.push(events_sets);
		if (skill === 0 && area[key]) result.push(events_sets);
		}
	}
	...
	if (result == false) {
		$gameMap.initColorArea();
		$gameMap.showRangeArea(event, skill);
		$gameMap.showEffectArea(x, y, this._srpgSkillWindow.item());
    ...
    Scene_Map.prototype.setSrpgAction = function (event, skill, x, y) {
        ...
        var events = $gameMap.sortEventsDistance($gameMap.srpgUnitsArea(null, true, event, skill), x, y);
  ...
  Scene_Map.prototype.setSrpgNextEnemyUnit = function() {
    if (this._actionEnemyUnits.length > 0) {
      ...
      if (event.isSrpgAutoBattle() && !battler.isDead()) {
        ...
        $gamePlayer.setSrpgCameraEvent(event);      // カメラ移動
        this._srpgStatusWindow.setSrpgUnit(event);
		this._srpgStatusWindow.user().SrpgEventFilter(false);
        if (event.checkSearch()) {
          battler.srpgMakeActions();                // 行動を作成
          if (battler.numActions() > 0) {
			  this._srpgTurnState = 1;
		      this._srpgStatusWindow.user().SrpgEventFilter(true);
		  }
        }
      }
    } else {
  ...
  Scene_Map.prototype.updateSrpgEnemyMoveReady = function(event) {
    ...
    if (destination) {
      ...
      var action = event.srpgBattler().currentAction();
	  if (!action.item()) return false; // 对于无法行动情况下的处理
